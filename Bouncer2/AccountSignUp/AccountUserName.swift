...
class AccountUserName: UIViewController{
    ...
    fileprivate let textField: UITextField = {
        ...
        textField.placeholder = "My Username"
    ...
    fileprivate func continueWith(_ input: String){
        ...
        Task{
            do{
                ...
                try await AccountInfo.process(input: input.lowercased(), .uploadUsername)
                AccountInfo.storeValue(input, DefaultsKeys.displayName)
                
            }catch{
                if error is AuthManager.AuthError{
                    self.showMessage(withTitle: "", message: "'\(input)' is already taken. Choose a different username.")
                }
                indicator.stopAnimating()
                indicator.removeFromSuperview()
...
extension AccountUserName: UITextFieldDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if string.count > 0{
            let set = CharacterSet(charactersIn: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLKMNOPQRSTUVWXYZ._1234567890")
            let letterRange = string.rangeOfCharacter(from: set)
            let onlyLetters = (letterRange != nil)
            
            if range.lowerBound == 20{
                textField.shake()
                return false
            }
